...
- [必要なもの](#必要なもの)
  ...
  - [実装する部品（電子部品など）](#実装する部品電子部品など)
  - [ケーブル](#ケーブル)
  - [使用する部品（機構部品など）](#使用する部品機構部品など)
  - [そのほか必要なもの](#そのほか必要なもの)
  - [工具](#工具)
- [本体の作成](#本体の作成)
  - [ケーブル加工](#ケーブル加工)
...
- はんだ付け用治具1,2はメイン基板とサイドキーパッド基板のはんだ付けの際のみに使用します。
...
![01](1.0/doc/img/cable/cable.png)
4 について
...
必要であればお使いの3Dプリンタに合わせてモデルの各種寸法は元となるstepファイルを編集し、適宜調整してください。
事前にサポート材をすべて除去してください．
また，ピンバイス等を用いてネジを通す穴をネジが通るように加工してください．

1. インサートナットを熱圧入します．
   本体底面部においては次の 8 箇所にインサートナットを圧入します．  
   ![本体底面部インサートナット](1.0/doc/img/3dprint/insert_bottom.png)  
   本体カバーにおいては次の 1 箇所にインサートナットを圧入します．  
   ![本体カバーインサートナット](1.0/doc/img/3dprint/insert_cover.jpg)  
   ボール保持部においては次の 2 箇所にインサートナットを圧入します．  
   ![ボール保持部インサートナット](1.0/doc/img/3dprint/insert_ball.jpg)  
   追加キーパッドモジュール底面部においては次の 2 箇所にインサートナットを圧入します．  
   ![追加キーパッドモジュール底面部インサートナット](1.0/doc/img/3dprint/insert_add_key_pad.jpg)
2. 磁石を接着します．
   本体底面部において，次の二か所に磁石を接着します．
   接着剤を穴の底面と側面に塗布し，磁石を接着します．
   この際に，二つの磁石の極が異なるように配置してください．  
   ![本体底面部磁石](1.0/doc/img/3dprint/magnet_bottom.jpg)

debug カバーにおいて次の二か所に磁石を接着します．
この際に，本体底面部と引き合う向きで磁石を接着します．
![debugカバー磁石](1.0/doc/img/3dprint/magnet_debug.jpg)

3. ゴム足を取り付けます．
   本体底面部，追加キーパッドモジュール底面部にゴム足を接着します．
   くぼみがあるところにゴム足を接着します．
   ゴム足がはがれることが多かったので接着剤を使用することを推奨します．
   ![本体底面部ゴム足](1.0/doc/img/3dprint/rubber_bottom.jpg)
   ![追加キーパッドモジュール底面部ゴム足](1.0/doc/img/3dprint/rubber_add_key_pad.jpg)
...
表面  
![センサ基板表面](1.0/doc/img/sensor_board/sensor_board_front.jpg)  
裏面  
![センサ基板裏面](1.0/doc/img/sensor_board/sensor_board_back.jpg)
...
1. PMW3360 の実装
   PMW3360 を実装します．向きに注意してください．正しい向きは．画像を参照してください。裏面からセンサを基板に差し込み，表面ではんだ付けします．  
   ![センサ基板PMW3360実装裏](1.0/doc/img/sensor_board/PMW3360_back.jpg)  
   ![センサ基板PMW3360実装表](1.0/doc/img/sensor_board/PMW3360_front.jpg)

2. コネクタの実装
   次に，コネクタを実装します．
   裏面からコネクタを基板に差し込み，表面ではんだ付けします．  
   ![センサ基板コネクタ実装](1.0/doc/img/sensor_board/conn.jpg)
3. レンズの取り付け
   PMW3360 のレンズを取り付けます．  
   センサのカプトンテープを剥がし，レンズをセンサに取り付けます．  
   ![センサ基板レンズ取り付け](1.0/doc/img/sensor_board/lens.jpg)  
   センサから飛び出ているガイドポスト二か所を半田ごてで溶かし，レンズを固定します．  
   ![センサ基板レンズ固定](1.0/doc/img/sensor_board/lens_melt.jpg)

#### メイン基板

表面  
![メイン基板表面](1.0/doc/img/main_board/main_board_front.jpg)  
裏面  
![メイン基板裏面](1.0/doc/img/main_board/main_board_back.jpg)
...
1. スイッチの実装
   ...
   ![メイン基板スイッチ実装](1.0/doc/img/main_board/BootReset.jpg)

2. PMW3360 の実装
   次に，PMW3360 を実装します．表面で足をはんだ付けします．向きに注意してください．向きを基板裏面のシンボルに合わせます．裏面からセンサを基板に差し込み，表面ではんだ付けします．  
   ![メイン基板PMW3360実装表](1.0/doc/img/main_board/PMW3360_front.jpg)  
   ![メイン基板PMW3360実装裏](1.0/doc/img/main_board/PMW3360_back.jpg)

3. コネクタの実装
次に，コネクタを実装していきます．  
初めに，センサモジュールとの接続用のコネクタ(S6B-PH-KL)を実装します.
表面からコネクタを基板に差し込み，裏面ではんだ付けします．  
![メイン基板センサコネクタ実装](1.0/doc/img/main_board/conn_sensor_front.jpg)  
次に，サイドキーパッドとの接続用のコネクタ(S8B-PH-KL)と OLED モジュールとの接続コネクタ(S6B-PH-KL)を実装します．裏面からコネクタを基板に差し込み，表面ではんだ付けします．  
![メイン基板サイドOLEDコネクタ実装](1.0/doc/img/main_board/conn_side_oled_back.jpg)  
次に，親指モジュールとの接続用のコネクタ(B6B-PH-KL)を実装します．裏面からコネクタを基板に差し込み，表面ではんだ付けします．  
![メイン基板親指コネクタ実装](1.0/doc/img/main_board/conn_thumb.jpg)  
任意でデバッグ用のコネクタを実装します．表面からコネクタを基板に差し込み，裏面ではんだ付けします．
<!--
![メイン基板デバグコネクタ実装](1.0/doc/img/main_board/conn_debug.jpg)
-->
...
表面  
![サイドキーパッド基板表面](1.0/doc/img/side_board/side_key_pad_board_front.jpg)  
裏面  
![サイドキーパッド基板裏面](1.0/doc/img/side_board/side_key_pad_board_back.jpg)
...
裏面に部品を実装します．
LED,ダイオード，MX ソケットを実装します．LED，ダイオード実装の際には向きに注意してください．正しい向きは画像を参照してください．  
![サイドキーパッド基板裏面実装](1.0/doc/img/side_board/back_imple.jpg)  
![RGBLED注意](1.0/doc/img/side_board/LED_attention.jpg)  
![ダイオード注意](1.0/doc/img/side_board/diode_attention.jpg)

#### 親指モジュール基板

表面  
![親指モジュール基板表面](1.0/doc/img/thumb_board/thumb_board_front.jpg)  
裏面  
![親指モジュール基板裏面](1.0/doc/img/thumb_board/thumb_board_back.jpg)
...
![親指モジュール基板裏面実装](1.0/doc/img/thumb_board/back_imple.jpg)  
![RGBLED注意](1.0/doc/img/side_board/LED_attention.jpg)

接続用のコネクタを実装します．裏面からコネクタを基板に差し込み，表面ではんだ付けします．  
![親指モジュール基板コネクタ実装](1.0/doc/img/thumb_board/conn.jpg)

#### OLED モジュール基板

表面  
![OLEDモジュール基板表面](1.0/doc/img/oled_board/oled_board_front.jpg)  
裏面  
![OLEDモジュール基板裏面](1.0/doc/img/oled_board/oled_board_back.jpg)
...
1. スイッチの実装
   表面実装スイッチを表面に実装します．  
   ![OLEDモジュール基板表面実装](1.0/doc/img/oled_board/SW.jpg)

2. コネクタの実装
   次に，コネクタを実装します．裏面からコネクタを基板に差し込み，表面ではんだ付けします．  
   ![OLEDモジュール基板コネクタ実装](1.0/doc/img/oled_board/conn.jpg)

3. OLED の実装
   次に，OLED を実装します．  
   OLED にピンヘッダをはんだ付けします．  
   ![OLEDピンヘッダはんだ付け](1.0/doc/img/oled_board/oled_header.jpg)  
   ...
   ![OLED注意](1.0/doc/img/oled_board/oled_para.jpg)

#### 追加キーパッドモジュール基板

表面  
![追加キーパッドモジュール基板表面](1.0/doc/img/add_key_pad_board/add_key_pad_board_front.jpg)  
裏面  
![追加キーパッドモジュール基板裏面](1.0/doc/img/add_key_pad_board/add_key_pad_board_back.jpg)
...
1. 表面実装部品の実装
   ...
   ![追加キーパッドモジュール基板裏面実装](1.0/doc/img/add_key_pad_board/back_imple.jpg)  
   ![ダイオード注意](1.0/doc/img/side_board/diode_attention.jpg)

2. ケーブルの取り付け
   ケーブル 4 をはんだ付けします．ケーブルの向きに注意してください．ケーブルが外側を向きようにします．また，基板を裏面から見た際にハウジングが見える向きにします．  
   ![追加キーパッドモジュール基板ケーブル4](1.0/doc/img/add_key_pad_board/cable4.jpg)

ケーブルを反対側に折り曲げます．  
![追加キーパッドモジュール基板ケーブル4折り曲げ](1.0/doc/img/add_key_pad_board/cable4_bend.jpg)

#### メイン基板とサイドキーパッド基板の配線

メイン基板とサイドキーパッド基板を接続します．
本体底面部にメイン基板とサイドキーパッド基板を取り付けます．固定には M2 4mm ネジを 4 本使用します．  
![メイン基板サイドキーパッド基板取り付け](1.0/doc/img/main_side_board/mount.jpg)  
ジャンパワイヤを 7 本基板表側から差し込みます．  
![メイン基板サイドキーパッド基板配線](1.0/doc/img/main_side_board/insart.jpg)  
この状態でマスキングテープ等で固定します．  
![メイン基板サイドキーパッド基板配線](1.0/doc/img/main_side_board/tape.jpg)  
メイン基板とサイドキーパッド基板を外し，はんだ付け用治具 1,2 に取り付けます．  
![メイン基板サイドキーパッド基板配線](1.0/doc/img/main_side_board/jig.jpg)  
基板の裏側からジャンパワイヤをはんだ付けします．  
![メイン基板サイドキーパッド基板配線](1.0/doc/img/main_side_board/solder.jpg)  
余分なジャンパワイヤを切断します．  
![メイン基板サイドキーパッド基板配線](1.0/doc/img/main_side_board/cut.jpg)

### 組み立て

追加キーパッドモジュール底面部にジョイントを取り付けます．ねじは M3 8mm ネジを使用します．また，ジョイントの上下を M3 ワッシャで挟み，M3 ナットで締結します．  
![追加キーパッドモジュール底面部ジョイント](1.0/doc/img/assemble/joint1.jpg)  
本体底面部とジョイントを締結します．ねじは M3 8mm ネジを使用します．また，ジョイントの上下を M3 ワッシャで挟み，M3 ナットで締結します．  
![追加キーパッドモジュール底面部ジョイント](1.0/doc/img/assemble/joint2.jpg)
...
![基板接続](1.0/doc/img/assemble/board_conn.jpg)

基板を本体底面部，追加キーパッドモジュール底面部にに取り付けます．
締結には M2 4mm ネジを使用します．締結箇所は次の 6 箇所です．
![基板取り付け](1.0/doc/img/assemble/board_mount.jpg)

本体カバーを取り付けます．
この際に M3 ナットの向きを調整してください．
![本体カバー取り付け](1.0/doc/img/assemble/cover_mount.jpg)

ボール保持部，ボール保持部カバーを取り付けます．
![ボール保持部取り付け](1.0/doc/img/assemble/ball_mount.jpg)
...
![ねじ締結場所1](1.0/doc/img/assemble/screw1.png)  
![ねじ締結場所2](1.0/doc/img/assemble/screw2.png)

OLED モジュールカバーを取り付けます．締結には M2 8mm ネジを使用します．  
![OLEDモジュールカバー取り付け](1.0/doc/img/assemble/oled_cover_mount.jpg)

親指モジュールカバーを取り付けます．締結には M2 6mm ネジを使用します．  
![親指モジュールカバー取り付け](1.0/doc/img/assemble/thumb_cover_mount.jpg)

サイドキーパッドカバーを取り付けます．  
![サイドキーパッドカバー取り付け](1.0/doc/img/assemble/side_key_pad_cover_mount.jpg)

キースイッチを取り付けます．  
![キースイッチ取り付け](1.0/doc/img/assemble/key_switch_mount.jpg)

キーキャップを取り付けます．  
![キーキャップ取り付け](1.0/doc/img/assemble/key_cap_mount.jpg)
